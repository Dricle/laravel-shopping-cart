...
use Dricle\Cart\Exceptions\InvalidConditionException;
use Dricle\Cart\Exceptions\InvalidItemException;
use Dricle\Cart\Exceptions\UnknownModelException;
use Dricle\Cart\Helpers\Helpers;
use Dricle\Cart\Validators\CartItemValidator;
...
class Cart
{
    ...
    public function update($id, $data)
    {
        ...
        foreach ($data as $key => $value) {
            if ($key == 'quantity') {
                ...
                // Merge new attributes with existing ones instead of replacing
                $existingAttributes = $item[$key]->toArray();
                $newAttributes = array_merge($existingAttributes, $value);
                $item[$key] = new ItemAttributeCollection($newAttributes);
            } else {
                $item[$key] = $value;
            }
        }
    ...
    public function addItemCondition($productId, $itemCondition)
    {
        if ($product = $this->get($productId)) {
            $conditionInstance = '\\Dricle\\Cart\\CartCondition';

            if ($itemCondition instanceof $conditionInstance) {
                // we need to copy first to a temporary variable to hold the conditions
                // to avoid hitting this error "Indirect modification of overloaded element of Dricle\Cart\ItemCollection has no effect"
    ...
    public function condition($condition)
    {
        ...
        if (! $condition instanceof CartCondition) {
            throw new InvalidConditionException('Argument 1 must be an instance of \'Dricle\Cart\CartCondition\'');
        }
