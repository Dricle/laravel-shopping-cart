...
class CartTest extends PHPUnit\Framework\TestCase
{
    ...
    public function test_cart_update_with_attribute_then_attributes_should_be_still_instance_of_item_attribute_collection()
    {
        ...
        $this->assertInstanceOf('Dricle\Cart\ItemAttributeCollection', $item->attributes);
    ...
    {
        $this->expectException('Dricle\Cart\Exceptions\InvalidItemException');
    ...
    public function test_cart_can_add_items_as_array_with_associated_model()
    {
        ...
        $this->assertEquals(456, $this->cart->getContent()->first()['id'], 'The first content must have ID of 456');
        $this->assertEquals('Sample Item', $this->cart->getContent()->first()['name'], 'The first content must have name of "Sample Item"');
        $this->assertInstanceOf('Dricle\Tests\helpers\MockProduct', $addedItem->model);
    }
    ...
    public function test_cart_can_add_items_with_multidimensional_array_with_associated_model()
    {
        ...
        foreach ($content as $item) {
            $this->assertInstanceOf('Dricle\Tests\helpers\MockProduct', $item->model);
        }
